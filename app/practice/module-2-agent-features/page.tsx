...
export default function Module2Practice() {
  ...
  const isDisabled = !name || !email || !message || !isValidEmail
...
const PricingCard = ({
  ...
  return (
    <div
      ...
      className={`border p-6 rounded-2xl shadow-md text-center transition transform hover:scale-105 ${
        featured
    ...
    >
      ...
      <p className={featured ? "text-white" : "text-gray-600"}>
        {description}
      </p>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-8">
          ...
          <section className="bg-white p-6 rounded-lg shadow border-2 border-yellow-500">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 scale-105 z-10">
  <PricingCard
  title="Basic"
  price="$9/mo"
  description="Great for beginners getting started"
>
...
function BrokenCalculator() {
  ...
  const calculate = async () => {
    try {
      const { n1, n2 } = await parseNumbers()
      const sum = n1 + n2
      setResult(sum)
    } catch (error: any) {
...
function calculateDiscount(price: number, discountPercent: number): number {
  ...
  return price - discountAmount
    ...
    // Tests (simulating /tests output)
console.assert(calculateDiscount(100, 10) === 90, '10% discount failed')
console.assert(calculateDiscount(100, 0) === 100, '0% discount failed')
console.assert(calculateDiscount(100, 100) === 0, '100% discount failed')
console.assert(calculateDiscount(100, 150) === 100, 'invalid discount failed')
console.assert(calculateDiscount(-100, 10) === -100, 'negative price failed')
