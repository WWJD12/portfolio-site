...
export default function Module4Practice() {
  ...
  const NotificationBadge = ({ count, maxCount = 99 }: NotificationBadgeProps) => {
    ...
    return <span className="px-2 py-1 text-xs bg-red-500 text-white rounded-full">{display}</span>
  }
    ...
    return (
      <div className="space-y-1">
        ...
        <div
          ...
          aria-valuemax={100}
        >
          <div className="h-4 bg-blue-500 rounded" style={{ width: `${safeValue}%` }} />
        </div>
      </div>
    )
  }
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        <div className="bg-white p-6 rounded shadow w-full max-w-md">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={onClose} aria-label="Close modal" className="text-gray-500">
              ✕
            </button>
          </div>
          ...
          {/* Note: In a real app, you'd trap focus inside the modal */}
        </div>
      </div>
    )
  }
...
`}
        ...
        <section className="bg-white p-6 rounded-lg shadow mb-10 border-l-4 border-green-400">
          ...
          <div className="border-2 border-green-400 rounded p-4 bg-green-50">
            ...
            <ProgressBar value={25} label="Profile Completion" />
            <ModalDialog open={true} onClose={() => {}} title="Example Modal">
