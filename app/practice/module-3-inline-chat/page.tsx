...
// 🔎 Filter by category
const filterByCategory = (items: Item[], filter: string): Item[] => {
...
// 📦 Only in-stock items
const filterInStock = (items: Item[]): Item[] => {
...
// 💰 Format + compute values
const formatItems = (items: Item[]): ProcessedItem[] => {
  return items.map(item => {
    const discountedPrice =
      item.price > 2 ? item.price * 0.9 : item.price

    return {
      ...item,
      discountedPrice,
      formattedPrice: `$${discountedPrice.toFixed(2)}`,
      isOnSale: item.price > 2,
    }
  })
}
...
function MessyComponent() {
  ...
  const processedItems = useMemo(() => {
    let result = filterByCategory(items, filter)
  ...
  return (
    <div className="space-y-6">
      <p className="text-gray-600">
        Clean, refactored, and optimized component ✨
      </p>
